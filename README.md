...
Install or upgrade from a [release](https://github.com/abdheshnayak/kaja-helm/releases) tarball or from the [GitHub Container Registry](https://github.com/abdheshnayak/kaja-helm/pkgs/container/kaja-agent) (OCI). Use `helm upgrade --install` so the same command is idempotent (installs if missing, upgrades if already installed).

**Release tarball** (works whenever a [release](https://github.com/abdheshnayak/kaja-helm/releases) exists):

```bash
helm upgrade --install kaja-agent https://github.com/abdheshnayak/kaja-helm/releases/download/v0.0.1/kaja-agent-0.0.1.tgz \
  --namespace kaja \
  --create-namespace \
...
Replace `v0.0.1` and `kaja-agent-0.0.1.tgz` with the [release](https://github.com/abdheshnayak/kaja-helm/releases) you want.

**OCI (ghcr.io):**

```bash
helm upgrade --install kaja-agent oci://ghcr.io/abdheshnayak/kaja-agent \
  --version 0.0.1 \
  --namespace kaja \
  --create-namespace \
  --set env.clusterId=mycluster \
...
Replace `0.0.1` with the [release](https://github.com/abdheshnayak/kaja-helm/releases) version. If you see `not found`: ensure the Release workflow ran for that tag and the “Push chart to OCI” step succeeded; if the package is private, use the tarball or make the [package](https://github.com/abdheshnayak/kaja-helm/pkgs/container/kaja-agent) public.
...
2. The workflow will set the chart version from the tag, lint and package the chart, push it to the [GitHub Container Registry](https://github.com/abdheshnayak/kaja-helm/pkgs/container/kaja-agent) as `ghcr.io/<owner>/kaja-agent`, and create a [GitHub Release](https://github.com/abdheshnayak/kaja-helm/releases) with `kaja-agent-<version>.tgz` attached.
